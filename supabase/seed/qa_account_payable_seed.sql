-- ============================================================================
-- QA seed - Account Payable Test Plan results (re-runnable, ASCII-safe)
-- Jalankan di Supabase SQL Editor. Tampil di /finance/qa.
-- Ringkasan: 58 cases . 56 pass . 0 fail . 2 blocked . 96.55% . PARTIAL
-- ============================================================================
DELETE FROM public.qa_test_runs
 WHERE tenant_id = '00000000-0000-0000-0000-000000000001'
   AND module = 'account-payable'
   AND title = 'Account Payable (Pengelolaan Tagihan) - Test Plan v1';
...
WITH r AS (
  ...
  VALUES
    ('00000000-0000-0000-0000-000000000001', 'account-payable',
     'Account Payable (Pengelolaan Tagihan) - Test Plan v1',
     'Eksekusi 58 test case (45 otomatis via node:test pada lib/ap-logic.ts, 13 integration/code-review). Mencakup US-001 sampai US-005: input & duplikat, workflow approval, AP aging, cash-out forecast, GL posting.',
     'Node 22 + node:test (logic) / Next.js 16 route review / Supabase public schema',
...
( VALUES
    ...
    ('TC-028', 'US-003', 'aging: 120 days overdue -> over-90 bucket', 'functional', 'Automated', 'Medium', 'Assertion holds', 'Matched expected', 'PASS', NULL, 28),
    ...
    ('IC-04', 'US-001', 'Validasi field wajib (no_invoice, tgl, pihak_ketiga, minimal 1 item) -> 400', 'integration', 'Code Review', 'Medium', 'HTTP 400 bila kurang', 'OK', 'PASS', NULL, 49),
