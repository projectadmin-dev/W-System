...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!notes) return NextResponse.json({ error: 'Alasan penolakan wajib diisi' }, { status: 400 })
    ...
    if (pu.status !== 'PENDING_APPROVAL') {
      return NextResponse.json({ error: 'Hanya dokumen PENDING_APPROVAL yang bisa ditolak' }, { status: 422 })
    }
