...
import { Trash2, Plus, Calculator, Save, ArrowLeft } from 'lucide-react';
...
export default function EditProjectPage() {
  ...
  const [deductions, setDeductions] = useState({ pajak: 11, founderFee: 3, managementFee: 0, seFee: 0 });
  const [topp, setTopp] = useState({ cogsPct: 20, opexPct: 80 });
    ...
    return (
      <div className="p-4 lg:p-6 flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          <Calculator className="h-8 w-8 animate-spin mx-auto mb-2 text-blue-500" />
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">Edit data proyek komersial</p>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <Button variant="outline" asChild>
          <Link href="/commercial/projects">Cancel</Link>
        </Button>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <div>
              ...
              <Input value={project.projectName} onChange={(e) => setProject((p) => ({ ...p, projectName: e.target.value }))} placeholder="Project Name" />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">PIC</label>
              <Input value={project.pic} onChange={(e) => setProject((p) => ({ ...p, pic: e.target.value }))} placeholder="Nama PIC" />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">Status</label>
              <Select value={project.status} onValueChange={(v) => setProject((p) => ({ ...p, status: v }))}>
            ...
            <div>
              <Badge className={`${STATUS_COLORS[project.status] || 'bg-gray-600'} text-white`}>{project.status}</Badge>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6">
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Project Type</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              <CardTitle className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Manpower Calculator</CardTitle>
              <Button onClick={addRow} size="sm" variant="outline">
                <Plus className="h-4 w-4 mr-1" /> Tambah Manpower
              </Button>
            </CardHeader>
            <CardContent>
              ...
              <div className="space-y-2">
                {rows.map((row, idx) => {
                  ...
                  const calc = entry ? { publish: entry.publishRate * (row.qty || 0) * (row.months || 0) } : { publish: 0 };
                  const roleOptions = getRoles(project.type, row.group);

                  return (
                    <div key={row.id} className="grid grid-cols-1 sm:grid-cols-[40px_1fr_1fr_1fr_70px_80px_100px_40px] gap-2 items-center bg-muted/40 rounded-md p-2 sm:p-0 sm:bg-transparent">
                      <div className="text-center text-sm text-muted-foreground font-bold hidden sm:block">{idx + 1}</div>
                      <div>
                        <label className="sm:hidden text-xs text-muted-foreground">Group</label>
                        <Select value={row.group || 'ALL'} onValueChange={(v) => updateRow(row.id, 'group', v === 'ALL' ? '' : v)}>
                      ...
                      <div>
                        ...
                        <Select value={row.role || 'ALL'} onValueChange={(v) => updateRow(row.id, 'role', v === 'ALL' ? '' : v)}>
                          <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="-- Role --" /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="ALL">-- Role --</SelectItem>
                            {roleOptions.map((r) => (<SelectItem key={`${r.group}-${r.role}`} value={r.role}>{r.role}</SelectItem>))}
                          </SelectContent>
                        </Select>
                      </div>
                      <div>
                        <label className="sm:hidden text-xs text-muted-foreground">Nama</label>
                        <Input className="h-8 text-xs" value={row.nama} onChange={(e) => updateRow(row.id, 'nama', e.target.value)} placeholder="Nama" />
                      </div>
                      <div>
                        <label className="sm:hidden text-xs text-muted-foreground">Qty</label>
                        <Input type="number" min={1} className="h-8 text-xs text-center" value={row.qty} onChange={(e) => updateRow(row.id, 'qty', parseInt(e.target.value) || 0)} />
                      </div>
                      <div>
                        <label className="sm:hidden text-xs text-muted-foreground">Months</label>
                        <Input type="number" min={0.1} step={0.1} className="h-8 text-xs text-right" value={row.months} onChange={(e) => updateRow(row.id, 'months', parseFloat(e.target.value) || 0)} />
                      </div>
                      <div className="text-right text-sm font-bold text-blue-500 hidden sm:block">{fmtIDR(calc.publish)}</div>
                      <div className="flex justify-end">
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400 hover:text-red-300" onClick={() => removeRow(row.id)}>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Cost Deductions (% dari Publish Rate)</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {([
                  { key: 'pajak' as const, label: 'Pajak (%)' },
                  ...
                  <div key={field.key}>
                    ...
                    <Input type="number" min={0} max={100} step={0.1} value={deductions[field.key]} onChange={(e) => setDeductions((d) => ({ ...d, [field.key]: parseFloat(e.target.value) || 0 }))} />
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Alokasi TOPP (% dari Sales Project)</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-muted-foreground mb-1 block">COGS TOPP (%)</label>
                  <Input type="number" min={0} max={100} step={0.1} value={topp.cogsPct} onChange={(e) => setTopp((t) => ({ ...t, cogsPct: parseFloat(e.target.value) || 0 }))} />
                </div>
                <div>
                  <label className="text-xs text-muted-foreground mb-1 block">OPEX Allocation (%)</label>
                  <Input type="number" min={0} max={100} step={0.1} value={topp.opexPct} onChange={(e) => setTopp((t) => ({ ...t, opexPct: parseFloat(e.target.value) || 0 }))} />
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-4">
          <Card className="border-blue-500/50">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Quotation Publish <span className="text-blue-400">(Rencana Awal)</span></CardTitle>
            </CardHeader>
            <CardContent>
              <Input value={quotationRaw} onChange={(e) => handleCurrencyInput(e.target.value, setQuotationRaw)} placeholder="IDR 0" className="text-lg font-bold" />
          ...
          <Card className="border-emerald-500/50">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Actual Deal <span className="text-emerald-400">(Realisasi)</span></CardTitle>
            </CardHeader>
            <CardContent>
              <Input value={actualDealRaw} onChange={(e) => handleCurrencyInput(e.target.value, setActualDealRaw)} placeholder="IDR 0" className="text-lg font-bold" />
              <div className="mt-2 space-y-1 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">Discount dari Publish:</span><span className="font-semibold">{pct(summary.discPublish)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">Discount dari Special Rate:</span><span className="font-semibold text-yellow-400">{pct(summary.discSpecial)}</span></div>
              </div>
            </CardContent>
          </Card>

          <div className="flex gap-2">
            <Button onClick={saveChanges} className="flex-1 bg-emerald-600 hover:bg-emerald-700">
              <Save className="h-4 w-4 mr-1" /> Save Changes
            </Button>
          </div>

          <SummaryCard label="Publish Rate (Rekomendasi)" value={summary.totalPublish} color="text-blue-400" />
          <SummaryCard label="HPP Total (Batas Bawah)" value={summary.totalHPP} color="text-red-400" />
          <SummaryCard label="Special Rate Total (Batas Atas)" value={summary.totalSpecial} color="text-yellow-400" />
          <SummaryCard label="Sales Project (setelah deduction)" value={summary.salesProject} color="text-blue-400" />

          <Card className="bg-muted/30">
            <CardContent className="py-3 space-y-1 text-sm">
              <DeductRow label={`Pajak (${pct(deductions.pajak)})`} value={summary.deductionDetails.pajakVal} />
          ...
          <Card className="bg-muted/30">
            <CardContent className="py-3 space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-cyan-400">COGS TOPP ({pct(topp.cogsPct)})</span>
          ...
          <Card className="bg-muted/30">
            <CardContent className="py-3 space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">COGS TOPP per Timeline:</span>
                <span className="font-mono font-bold text-cyan-400">
                  {summary.maxMonths > 0 ? fmtIDR(summary.cogsAmount / summary.maxMonths) + ' / bulan' : 'N/A'}
                </span>
              </div>
              <div className="text-xs text-muted-foreground">COGS TOPP dibagi {summary.maxMonths} Month(s)</div>
            </CardContent>
          </Card>
